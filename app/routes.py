...
def index():
    ...
    posts = db.paginate(current_user.folowing_posts(), page = page, per_page=app.config['POSTS_PER_PAGE'], error_out=False)
...
@login_required
def edit_profile():
    form = EditProfileForm(current_user.username) # <-- Truyền original_username vào form
    if form.validate_on_submit(): # <-- Xử lý POST request hợp lệ
        current_user.username = form.username.data
        current_user.about_me = form.about_me.data
        db.session.commit()
        flash('Your changes have been saved.')
        return redirect(url_for('edit_profile'))
    elif request.method == 'GET': # <-- Xử lý GET request (lần đầu tải trang)
